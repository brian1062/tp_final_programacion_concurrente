...
public class PetriNet {
    ...
    /**
     * Checks if a transition is enabled in the Petri net by checking if all input places have enough tokens.
     *
    ...
    public boolean isTransitionEnabled(int transitionIndex) {
        for (int placeIndex = 0; placeIndex < places.size(); placeIndex++) {
            // Verifies that the marking in the place is at least equal to the value in the input incidence matrix
    ...
    private void updateEnabledTransitions() {
        ...
        for (int transitionIndex = 0; transitionIndex < incidenceMatrixIn[0].length; transitionIndex++) {
