...
public class PetriNetLogReader {
  ...
  public static void validateLogFile(String logFilePath, PetriNetConf petriNetConf)
      throws IOException {
    ...
    for (LogEntry entry : logEntries) {
      ...
      if (!isTransitionEnabled) {
        throw new IllegalStateException(
            "In line "
                + (logEntries.indexOf(entry) + 1)
                + ": Transition "
                + entry.transition
                + " is not enabled with marking "
                + Arrays.toString(marking));
      }

      previousMarking = marking;
      marking = entry.marking;

      boolean isMarkingCorrect =
          isMarkingCorrect(entry.transition, previousMarking, marking, petriNetConf);
      if (!isMarkingCorrect) {
        throw new IllegalStateException(
            "In line "
                + (logEntries.indexOf(entry) + 1)
                + ": Marking "
                + Arrays.toString(marking)
                + " is not correct after transition "
                + entry.transition
                + ". Previous marking: "
  ...
  private static boolean isTransitionEnabled(
      String transition, int[] marking, PetriNetConf petriNetConf) {
  ...
  private static boolean isMarkingCorrect(
      String transition, int[] previousMarking, int[] marking, PetriNetConf petriNetConf) {
    int transitionIndex = Integer.parseInt(transition.substring(1));
    ...
    for (int i = 0; i < marking.length; i++) {
      expectedMarking[i] =
          previousMarking[i]
              + petriNetConf.getIncidenceMatrixOut()[i][transitionIndex]
              - petriNetConf.getIncidenceMatrixIn()[i][transitionIndex];
    }
